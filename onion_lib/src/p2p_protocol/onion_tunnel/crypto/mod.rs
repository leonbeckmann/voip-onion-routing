...
mod tests {
    ...
    #[test]
    fn unit_test_data_decryptable() {
        let sym_key = vec![4; KEYSIZE];
        let iv = vec![5; IVSIZE];
        let data = b"Some Crypto TextSome Crypto Text".to_vec();

        let crypt1 = CryptoContext::new(sym_key.clone());
        let crypt2 = CryptoContext::new(sym_key.clone());

        let (enc_iv, enc_data) = crypt1.encrypt(&iv, &data);
        let (dec_iv, dec_data) = crypt2.decrypt(&enc_iv, &enc_data);
